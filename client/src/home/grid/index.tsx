        ...
        <motion.div key={`row` + i} className="w-16 h-8  border-l  border-slate-700 relative">
          {cols.map((_, j) => (
            <motion.div
              ...
              key={`col` + j}
              className="w-16 h-8  border-r border-t border-white opacity-50 relative"
            >
              {j % 2 === 0 && i % 2 === 0 ? (
                <svg
                  ...
                  strokeWidth="1.5"
                  stroke="currentColor"
                  className="absolute h-6 w-10 -top-[14px] -left-[22px] text-main-blue stroke-[1px] pointer-events-none"
                >
